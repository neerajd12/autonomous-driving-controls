 ...
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    for (int t = 0; t < N - 1; t++) {
      ...
      fg[0] += 95 * CppAD::pow(vars[a_start + t], 2);
    }
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  for(int i = 0; i < n_vars; i++) {
    ...
    } else if(i == delta_start || i < a_start) {
  ...
  vector<double> actuator_values = {
    (solution.x[delta_start]
      ...
      + solution.x[delta_start+4])/5,
    (solution.x[a_start]
      + solution.x[a_start+1]
      + solution.x[a_start+1]
      + solution.x[a_start+2]
      + solution.x[a_start+3]
      + solution.x[a_start+4])/5};
  mpc_x_vals.clear();
  mpc_y_vals.clear();
  for(int i=1; i<N; i++) {
